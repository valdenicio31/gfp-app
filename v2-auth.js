...
document.querySelector('#inviteCep').addEventListener('input',event=>{const v=digits(event.target.value).slice(0,8);event.target.value=v.replace(/(\d{5})(\d)/,'$1-$2');document.querySelector('#correiosLink').title=`Consultar o CEP ${event.target.value} no site oficial dos Correios`});
